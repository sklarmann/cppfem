...
void KernelShapes(prec &shape, prec &derivative, const prec &coor, const uint &number) {
	...
	case 0:
	{
		constexpr prec val1 = 3.0 / 2.0, val2 = 2.0;
		shape = -val2*std::sqrt(val1);
		derivative = 0.0;
		break;
	}
	case 1:
	{
		constexpr prec val1 = 5.0 / 2.0, val2 = 2.0;
		shape = -val2*std::sqrt(val1)*coor;
		derivative = -val2*std::sqrt(val1);
		break;
	}
	case 2:
	{
		constexpr prec val1 = 7.0 / 2.0, val2 = 1.0 / 2.0, val3 = 1.0, val4 = 5.0, val5=10.0;
		shape = -val2*std::sqrt(val1)*(coor*coor*val4 - val3);
		derivative = -val2*std::sqrt(val1)*val5*coor;
		break;
	}
	case 3:
