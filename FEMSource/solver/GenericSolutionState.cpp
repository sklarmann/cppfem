...
namespace FEMProject {
  ...
  template<typename prec, typename uint> 
  void FEMProject::GenericSolutionState<prec, uint>::assembleCsrMatrix(Eigen::Matrix<prec, Eigen::Dynamic, 1>& toInsert, Eigen::Matrix<prec, Eigen::Dynamic, 1>& residual, std::vector<DegreeOfFreedom<prec, uint> *>& Dofs)
  {
  ...
  template<typename prec, typename uint>
  void GenericSolutionState<prec, uint>::modifyLinked(
  ...
  template<typename prec, typename uint> 
  void GenericSolutionState<prec, uint>::compressVector(
    Eigen::Matrix<prec, Eigen::Dynamic, 1>& compVect, 
    Eigen::Matrix<prec, Eigen::Dynamic, 1>& uncompVect, 
    bool compr)
  {
    EquationHandler<prec,uint> *tempEq = this->pointers->getEquationHandler();
    std::vector<DegreeOfFreedom<prec, uint>>* Dofs= tempEq->getDofList();
    uint numDofs = Dofs->size();
    
    if(compr){
      compVect.resize(this->NumberOfActiveEquations);
      for (auto i=0;i<numDofs;++i){
        DegreeOfFreedom<prec,uint> *tempDof = &(*Dofs)[i];
        if(tempDof->getStatus()!=dofStatus::inactive) {
          compVect(tempDof->getEqId()) = uncompVect(tempDof->getId());
        }
      }
    } else {
      ...
      for (auto i=0;i<numDofs;++i){
        DegreeOfFreedom<prec,uint> *tempDof = &(*Dofs)[i];
        if(tempDof->getStatus()!=dofStatus::inactive) {
          uncompVect(tempDof->getId()) = compVect(tempDof->getEqId());
        }
      }
    }
  }
